...
export class Widgets extends Component {
  ...
  renderNameInput = ({ input, type, meta: { touched, error } }) => (
    <div>
      ...
      */}
      {touched &&
      error && (
        <div style={{ color: '#cc7a6f', margin: '-10px 0 15px', fontSize: '0.7rem' }}>{error}</div>
      )}
    </div>
  );
  ...
  render() {
    ...
    const {
      handleSubmit,
      invalid,
      widgets: { list, requesting, successful, messages, errors },
    } = this.props;
      ...
      <div className="widgets">
        <div className="widget-form">
          <form onSubmit={handleSubmit(this.submit)}>
            ...
            <Field name="size" type="number" id="size" className="number" component="input" />
